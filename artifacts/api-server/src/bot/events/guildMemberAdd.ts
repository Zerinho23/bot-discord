  ...
  export async function onGuildMemberAdd(member: GuildMember): Promise<void> {
    ...
    try {
      ...
      if (welcomeConfig?.enabled) {
        const rawDescription = (welcomeConfig.embedDescription ?? "Welcome to the server!")
          .replace("{user}", `<@${member.id}>`)
          .replace("{username}", member.user.username)
          .replace("{server}", member.guild.name)
          .replace("{memberCount}", String(member.guild.memberCount));

        const rawTitle = (welcomeConfig.embedTitle ?? `Welcome to ${member.guild.name}!`)
          .replace("{user}", member.user.username)
          .replace("{username}", member.user.username)
          .replace("{server}", member.guild.name)
          .replace("{memberCount}", String(member.guild.memberCount));

        // Resolve custom guild emojis :name: → <:name:id> or <a:name:id>
        const description = resolveEmojis(rawDescription, member.guild);
        const title = resolveEmojis(rawTitle, member.guild);
        ...
        if (welcomeConfig.embedFooter) {
          const rawFooter = welcomeConfig.embedFooter
            .replace("{user}", member.user.username)
            .replace("{server}", member.guild.name);
          embed.setFooter({ text: resolveEmojis(rawFooter, member.guild) });
        }
